...
csri_inst *csri_open_file(csri_rend *renderer, const char *filename,
                          struct csri_openflag *flags) {
  return nullptr;
  //  return subhelp_open_file(renderer, csri_open_mem, filename, flags);
}

csri_inst *csri_open_mem(csri_rend *renderer, const void *data, size_t length,
                         struct csri_openflag *flags) {
  csri_inst *rv;
  if (renderer != &csri_libass) return NULL;
  ...
  rv->ass_track =
      ass_read_memory(csri_libass.ass_library, (char *)data, length, "UTF-8");
  if (!rv->ass_track) {
...
void csri_render(csri_inst *inst, struct csri_frame *frame, double time) {
  ASS_Image *img = ass_render_frame(inst->ass_renderer, inst->ass_track,
                                    (int)(time * 1000), NULL);
  ...
  while (img) {
    ...
    if ((frame->pixfmt | 1) == CSRI_F__RGB || frame->pixfmt == CSRI_F_RGB) {
    ...
    dst = frame->planes[0] + img->dst_y * frame->strides[0] + img->dst_x * bpp;
    if (frame->pixfmt & 1) dst++;
    src = img->bitmap;
...
static csri_inst *libass_init_stream(csri_rend *renderer, const void *header,
                                     size_t headerlen,
                                     struct csri_openflag *flags) {
  csri_inst *rv;
  ...
  ass_process_codec_private(rv->ass_track, (char *)header, headerlen);
  return rv;
}

static void libass_push_packet(csri_inst *inst, const void *packet,
                               size_t packetlen, double pts_start,
                               double pts_end) {
  ass_process_chunk(inst->ass_track, (char *)packet, packetlen,
                    (int)(pts_start * 1000),
                    (int)((pts_end - pts_start) * 1000));
}

static struct csri_stream_ext streamext = {libass_init_stream,
                                           libass_push_packet, NULL};

void *csri_query_ext(csri_rend *rend, csri_ext_id extname) {
  if (!rend) return NULL;
  if (!strcmp(extname, CSRI_EXT_STREAM_ASS)) return &streamext;
  return NULL;
}

static struct csri_info csri_libass_info = {
    "libass", "0.9.x", "libass (the MPlayer SSA/ASS renderer, 0.9.x API)",
    "Evgeniy Stepanov", "Copyright (c) 2006, 2007 by Evgeniy Stepanov"};
...
csri_rend *csri_renderer_byname(const char *name, const char *specific) {
  if (strcmp(name, csri_libass_info.name)) return NULL;
  if (specific && strcmp(specific, csri_libass_info.specific)) return NULL;
  return &csri_libass;
}

csri_rend *csri_renderer_default() {
  csri_libass.ass_library = ass_library_init();
  if (!csri_libass.ass_library) return NULL;
...
csri_rend *csri_renderer_next(csri_rend *prev) { return NULL; }
